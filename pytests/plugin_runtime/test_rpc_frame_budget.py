...
def test_full_hook_budget_accounts_for_tools_metadata_and_preserves_original():
    ...
    result = fit_request_hook_kwargs("maisaka.planner.before_request", original, targets=targets)
...
def test_pure_text_hook_cannot_be_fixed_by_image_budget():
    with pytest.raises(FrameTooLargeError):
        fit_request_hook_kwargs(
            "maisaka.replyer.before_model_request", hook_kwargs(4, MAX_FRAME_SIZE), targets=[("test", "test", 60000)]
        )
